...
METHOD_CONFIGS = {
    ...
    'v3': {
        ...
        'has_scale': True,
        'scale_type': 'scalar',  
        'filename_suffix': 'mast3r_scaled_v3'
    },
    'v4': {
        'has_pointmap_error': True,
        'has_scale': True,
    ...
    'icp': {
        ...
        'has_scale': True,
        'scale_type': 'scalar', 
        'filename_suffix': 'mast3r_scaled_icp'
    }
}
...
def create_csv_with_headers(output_file, method_config):
    ...
    if method_config['has_pointmap_error']:
        base_headers.append('pointmap_error')
...
def save_result_csv(output_file, query_idx, anchor_idx, pose, statistics, method, status="OK"):
    ...
    # Add statistics (ensure we have enough values)
    base_stat_names = [
    ...
    for i, name in enumerate(base_stat_names):
        row_data[name] = statistics[i] if i < len(statistics) else np.nan

    # Add optional fields based on statistics length
    stat_idx = len(base_stat_names)
    if len(statistics) > stat_idx:
        # Check if this method has pointmap_error
        config = METHOD_CONFIGS[method]
        if config['has_pointmap_error']:
            row_data['pointmap_error'] = statistics[stat_idx]
            stat_idx += 1

        # Check if this method has scale values
        if config['has_scale']:
            if config['scale_type'] == 'vector':
                row_data['scale_x'] = statistics[stat_idx]
                row_data['scale_y'] = statistics[stat_idx + 1]
                row_data['scale_z'] = statistics[stat_idx + 2]
            elif config['scale_type'] == 'scalar':
                row_data['scale'] = statistics[stat_idx]
...
def estimate_and_evaluate_pose(method, mast3r_pts, lidar_pts, inlier_im1, K_new,
    ...
    config = METHOD_CONFIGS[method]
    if config['has_pointmap_error']:
        statistics.append(pointmap_error)
...
def process_pair(model, anchor, query, anchor_idx, query_idx, K, T_base_cam, T_cam_lidar, args, output_files):
    ...
    try:
        ...
        # Filter points to overlapping region
