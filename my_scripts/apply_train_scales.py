...
def parse_args():
    ...
    parser.add_argument('--scale_json', type=str, required=True, help='Path to JSON with per-scene scales')
...
def get_scale_from_json(method, scene_scales):
    if scene_scales is None:
        return None
    key_map = {
        'v2': 'mast3r_scaled_v2',
        'v3': 'mast3r_scaled_v3',
        'v4': 'mast3r_scaled_v4',
        'icp': 'mast3r_scaled_icp',
    }
    k = key_map[method]
    if k not in scene_scales:
        return None
    val = scene_scales[k]
    if isinstance(val, dict):
        return np.array([val.get('x'), val.get('y'), val.get('z')], dtype=float)
    else:
        return float(val)

def estimate_pose_for_method(method, inlier_im0, inlier_im1, pts3d_im0, scene_map, depth_map, K_new, scene_scales=None):
    if method == 'lidar':
        ...
        scale = get_scale_from_json(method, scene_scales)
        if scale is None:
            return None, None, None
...
def process_pair(model, anchor, query, anchor_idx, query_idx, K, T_base_cam, T_cam_lidar, args, output_files, scene_scales):
    try:
        ...
        for method, output_file in output_files.items():
            T, pts_for_depth, scale = estimate_pose_for_method(
                method, inlier_im0, inlier_im1, pts3d_im0, scene_map, depth_map, K_new, scene_scales=scene_scales
            )
            ...
            statistics = [
                n_matches, n_inliers,
                (len(pts_for_depth) if method == 'lidar' else np.nan),
            ...
            if config['has_pointmap_error']:
                if scale is not None:
                    scaled_pts3d = pts3d_im0 * scale
                else:
                    scaled_pts3d = pts3d_im0
                pointmap_errors = compute_pointmap_error(scaled_pts3d, scene_map)
                statistics.extend(pointmap_errors)

            if config['has_scale'] and scale is not None:
                if isinstance(scale, np.ndarray):
                    statistics.extend(scale.tolist())
                else:
                    statistics.append(scale)
...
def main():
    ...
    for dataset_scene in args.dataset_scenes:
        ...
        for anchor_idx, query_idx in tqdm(pairs, desc=f"Estimating poses for {dataset_scene}"):
            ...
            process_pair(model, anchor, query, anchor_idx, query_idx, K, T_base_cam, T_cam_lidar, args, output_files, scene_scales)
